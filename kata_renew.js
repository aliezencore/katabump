...
// mihomo 是否启动成功；未识别出节点时保持 false，Chrome 直连（勿删声明，下方启动 Chrome 前会读取）
...
// 解析 VMess 分享链接 (vmess:// + base64(JSON)) 为 mihomo vmess 节点
function parseVmessLinkToMihomoProxy(link, getUniqueName) {
    try {
        let payload = link.slice(link.indexOf('://') + 3).trim().split('#')[0];
        const json = JSON.parse(Buffer.from(payload.replace(/-/g, '+').replace(/_/g, '/'), 'base64').toString('utf8'));
        if (!json.add || !json.port || !json.id) return null;
        const q = (s) => String(s).replace(/\\/g, '\\\\').replace(/"/g, '\\"');
        const proxyNode = {
            // 隐私：节点名不使用分享链接中的 ps 备注（Actions 日志公开可见）
            name: getUniqueName('VMess'),
            type: 'vmess',
        ...
        if (json.tls === 'tls' || json.tls === true) proxyNode.tls = true;
        if (json.sni) proxyNode.servername = q(json.sni);
        if (json.net === 'ws') {
            proxyNode.network = 'ws';
            proxyNode['ws-opts'] = { path: q(json.path || '/') };
            if (json.host) proxyNode['ws-opts'].headers = { Host: q(json.host) };
        } else if (json.net === 'grpc') {
            proxyNode.network = 'grpc';
            proxyNode['grpc-opts'] = { 'grpc-service-name': q(json.path || '') };
        }
        return proxyNode;
    } catch (e) {
        return null;
...
// 自动解析 S5 节点文本（支持 VMess、Telegram socks、HTTP/HTTPS、标准 Socks5 及 IP:Port）
// 隐私：节点名统一为「类型-序号」，不携带分享链接备注/服务器地址等可识别信息（Actions 日志公开可见）
function parseS5TextToMihomoProxies(rawText) {
    const lines = rawText.split(/\r?\n/);
    const proxies = [];
    const seenNames = new Set();
    const seqByType = Object.create(null);
    ...
    function nextName(type) {
        ...
        return getUniqueName(`${type}-${seqByType[type]}`);
    }
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // 0. VMess 分享链接: vmess://base64(JSON)
        if (/^vmess:\/\//i.test(line)) {
            const proxyNode = parseVmessLinkToMihomoProxy(line, getUniqueName);
            if (proxyNode) {
        ...
        // 3. HTTP(S) 代理格式 (可带 | 备注，备注不进入日志): https://user:pass@host:port | 备注
...
async function setupSmartProxyPool(proxySource) {
    ...
    // 带凭据的代理 URL（https://user:pass@host:port、socks5://...）是"节点"而非"订阅链接"，
    // 不能当作远程订阅去 GET，否则解析失败会静默直连
    const looksLikeProxyUrl = /^socks5?:\/\//i.test(proxySource) ||
        /^https?:\/\/[^\/@:]+:[^\/@]+@[^\/@:]+(:\d+)?\/?$/i.test(proxySource);
    const isUrl = !looksLikeProxyUrl && /^https?:\/\//i.test(proxySource);
    let rawContent = '';
...
`;
        ...
        if (started) {
            ...
            if (proxies.length > 0) {
                ...
                if (testRes.healthy.length === 0) {
                    console.log('[智能代理] ⚠️ 订阅节点健康检查全部失败，已停用代理改用直连。');
                    IS_MIHOMO_ENABLED = false;
                }
                return {
                ...
                console.log('[智能代理] ⚠️ mihomo 已启动但未获取到节点，已停用代理改用直连。');
                IS_MIHOMO_ENABLED = false;
            }
        }
    }
    ...
    if (parsedProxies.length > 0) {
        console.log(`[智能代理] 成功解析并提取出 ${parsedProxies.length} 个代理节点 (含 VMess / TG Socks / HTTP / Socks5)...`);
...
`;
        ...
        if (started) {
            ...
            if (proxies.length > 0) {
                const testRes = await testMihomoProxies(proxies);
                if (testRes.healthy.length === 0) {
                    console.log('[智能代理] ⚠️ 节点健康检查全部失败，已停用代理改用直连。');
                    IS_MIHOMO_ENABLED = false;
                }
                return {
                    pool: testRes.healthy,
...
async function getMihomoProxies() {
    ...
    if (fs.existsSync(path.join(process.cwd(), 'sub1.yaml'))) {
        try {
            ...
            console.error('[代理池] sub1.yaml 已下载，长度', subContent.length, '字符（内容不在日志展示，避免泄露节点信息）');
        } catch(e) {}
    }
    ...
    try {
        ...
        console.error('[代理池] Mihomo 运行日志(末40行):\n', logTail);
    } catch (err) {}
            ...
            try {
                ...
                for (let attempt = 1; attempt <= RENEW_MAX_ATTEMPTS; attempt++) {
                    ...
                    if (await renewBtn.isVisible()) {
                        ...
                        console.log('未找到 Renew 按钮 (可能未到续期窗口；若长期未出现请核对账号与 serverId 是否匹配)。');
                        break;
                    }
                } 
