...
- 每 12 小时运行一次（UTC 23:20 / 11:20 = 北京时间 **07:20 / 19:20**），也可在 Actions 页面手动触发（可勾选 debug 模式）
...
1. **提前 10 分钟起跑（07:20）**：GitHub 定时任务常有几分钟到几十分钟延迟，留出余量避免踩线。
   若本次运行时窗口还差 ≤25 分钟开启，脚本会**在作业内等到窗口开启后再续期**（不会白跑一趟）。
2. **12 小时一次兜底**：万一定时任务被延迟或某次执行失败，最多 12 小时后就补上，而不是等一整天。
   两次运行之间，若窗口未到则直接跳过——此时连浏览器都不会启动（预检提前退出），开销很小。

> 频率取舍：12 小时一次（每天 2 次）已足够覆盖 GitHub 的调度抖动。
> 如需更保险可把 `.github/workflows/kata-renew.yml` 里的 cron 改成每 6 小时（`20 23,5,11,17 * * *`），
...
| `RENEW_EARLY_WAIT_MIN` | `25` | 距窗口多少分钟内就在本次作业里等到开启，而不是跳过 |
| `RENEW_RETRY_BUDGET_MIN` | `20` | 作业内轮询等窗口的预算（分钟） |
