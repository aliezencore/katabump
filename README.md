...
- 每 12 小时运行一次（UTC 21:20 / 09:20 = 北京时间 **05:20 / 17:20**），也可在 Actions 页面手动触发（可勾选 debug 模式）
- 登录（自动过 Cloudflare Turnstile）→ 进入服务器续期页 → 点击 Renew → 处理 ALTCHA 验证码 → 确认续期
- 站点续期周期为 **每 4 天一次**；未到窗口时脚本记录站点提示并跳过，到点自动续期，无需人工干预
- 续期结果写入 `renew_dates.json` 并自动 commit 回仓库

### 定时策略说明（为什么不是整点、也不是贴着 07:30 跑）

站点续期窗口按「上次续期成功那一刻 + 4 天」计算，因此脚本把**窗口时刻锚定在真实续期成功的瞬间**，
而不是只记一个日期。三个关键设计：

1. **提前约 2 小时起跑（北京 05:20）**：本仓库实测 GitHub 定时任务延迟 **102~131 分钟（中位约 111 分钟）**。
   若按 07:20 起跑，实际到达已是约 09:10 —— 窗口（07:30）过了 1 小时 40 分，只能事后补救。
   提前到 05:20 后，无论延迟 0 还是 2 小时，到达时刻都落在窗口之前。
2. **作业内等到窗口开启再续期**：到达后若窗口还没开，脚本会等到窗口开启那一刻再点 Renew
   （默认最多等 4 小时，见 `RENEW_EARLY_WAIT_MIN`），因此提前跑不会浪费，也不会早于窗口执行。
3. **12 小时一次兜底**：万一定时任务被延迟或某次执行失败，最多 12 小时后就补上，而不是等一整天。
   两次运行之间若窗口未到则直接跳过——此时连浏览器都不会启动（预检提前退出），开销很小。

> 频率取舍：12 小时一次（每天 2 次）已足够覆盖 GitHub 的调度抖动。
> 如需更保险可把 cron 改成每 6 小时（`20 21,3,9,15 * * *`，即北京 05:20/11:20/17:20/23:20），
> 代价是 Actions 运行次数翻倍（公开仓库免费，私有仓库注意每月 2000 分钟额度）。

### 窗口相关环境变量（可选）

都有内置默认值，**不配置也能正常运行**；如需覆盖，在 Secrets 里添加同名变量即可：

| 变量 | 默认值 | 说明 |
|------|--------|------|
| `SITE_TZ` | `Asia/Shanghai` | 站点时区。决定站点返回的「日期」如何换算成绝对时刻，避免 runner(UTC) 与本地的 8 小时错位 |
| `RENEW_CYCLE_HOURS` | `96` | 站点续期周期（小时），站点为每 4 天一次 |
| `RENEW_ANCHOR_TIME` | `07:30` | 旧数据没有「成功时刻」时，把站点日期当作该时刻开启窗口 |
| `RENEW_EARLY_WAIT_MIN` | `240` | 距窗口多少分钟内就在本次作业里等到开启，而不是跳过。需 ≥ cron 提前量 |
| `RENEW_RETRY_BUDGET_MIN` | `300` | 作业内等待窗口的预算（分钟），GitHub 单 job 上限 6 小时 |

### 本地自检

```bash
# 纯时间逻辑单测 + 用真实 renew_dates.json 的集成校验（零依赖，不需要装包）
node test/renew_window.test.js
node test/integration_renew_dates.test.js

# 干跑：只打印「本次会做什么」，不联网、不改 renew_dates.json
USERS_JSON='[{"username":"a@b.com","password":"x"}]' DRY_RUN=1 node kata_renew.js
```
